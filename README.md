...
This playbook is that pattern, extracted from a plugin that actually shipped it: [**engram**](https://github.com/nagisanzenin/engram) runs natively on **Claude Code, OpenAI Codex, OpenCode, Hermes Agent, Google Antigravity, OpenClaw, and Pi** — same skills, same engine, same user state, on all seven. The sixth is a chat gateway rather than a coding tool, which proved the pattern generalises past the terminal; the seventh reads the Agent Skills standard natively, which proved how cheap a port becomes when a platform meets the core where it already lives (one manifest key, one extension file, three prompt templates). Every rule in here carries a receipt: the real bug, review round, or user report that taught it. No rules without receipts.
...
| Read | For |
|---|---|
| [docs/01-anatomy.md](docs/01-anatomy.md) | The omni-repo shape: core vs glue, the seven-platform comparison table, the L0–L4 integration ladder |
...
| **Pi** | `pi install git:github.com/nagisanzenin/omniplugin` — no package.json here, so pi's conventional-dir scan picks up `skills/` from the clone, and the whole repo (with `docs/`) comes along |
...
| Pi | one `pi` key in package.json + one extension file (env export + nudge) + three prompt templates | L4 (v1.11.1, 2026-08-04 — transport verified 20/20 by a committed payload-capture harness, on two pi version lines) |

Shared and byte-identical across all seven: `skills/`, `agents/*.md` (as prompt source), `scripts/engram.py` (302 selftest checks at v1.11.1, same count everywhere), one state dir.

## Scope, honestly

This is distilled from **one plugin shipping seven platforms** (plus a researched survey of the rest of the landscape). It is a playbook, not a spec: platform loaders drift monthly, so every platform fact here is dated and versioned, and [CLAUDE.md](CLAUDE.md) tells future maintainers how to keep it true. When this playbook and a platform's current docs disagree, the platform docs win — then fix the playbook.
